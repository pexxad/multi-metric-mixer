...
import type { ApplicationRole, AssuranceLevel, AuthenticatedIdentity, WorkspaceContext } from '../persistence/identity-repository'
import type { AuthorizationTransaction } from './provider'
import type { RequestIdentity } from '../../shared/request-context'
export type { RequestIdentity } from '../../shared/request-context'
...
export class SessionService {
  ...
  constructor(private readonly database: BffDatabase, sessionSecret: string, private readonly ttlSeconds: number) {
...
export class AuthTransactionStore {
  ...
  constructor(private readonly database: BffDatabase, secret: string) { this.encryptionKey = createHash('sha256').update(secret).digest() }
