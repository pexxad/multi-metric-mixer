...
describe('Version 1 persisted user flows', () => {
  ...
  async function setup(subject = 'alice') {
    ...
    const context: RequestContext = await testContext(database, `request-${subject}`, {
  ...
  it('stores an idempotent chat exchange and context snapshot against an immutable Workflow version', async () => {
    ...
    const saved = await new WorkflowRepository(database).save(context, sampleWorkflow, 'agent')
    ...
    expect(await database.query.selectFrom('context_snapshots').select(({ fn }) => fn.countAll().as('count')).executeTakeFirst()).toMatchObject({ count: 1 })
  ...
  it('quarantines validated JSON/CSV as untrusted Artifacts and rejects dangerous CSV headers', async () => {
    ...
    const ingestion = new UploadIngestionService(new ArtifactRepository(database, contentStore), contentStore, {
  ...
  it('restores Workflow metadata and Artifact content after a local process restart', async () => {
    ...
    let database = await ApplicationDatabase.open({ kind: 'sqlite', filename }); databases.push(database)
    const context = await testContext(database, 'before-restart')
    const saved = await new WorkflowRepository(database).save(context, sampleWorkflow, 'manual')
    const { FileArtifactContentStore } = await import('./persistence/artifact-content-store')
    ...
    database = await ApplicationDatabase.open({ kind: 'sqlite', filename }); databases.push(database)
    const after = await testContext(database, 'after-restart')
